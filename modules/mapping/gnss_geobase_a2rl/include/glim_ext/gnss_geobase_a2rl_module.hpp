...
#include <common_msgs/msg/gnss.hpp>
 ...
 */
class GNSSGeobaseA2RL : public ExtensionModuleBase {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  GNSSGeobaseA2RL() : logger(create_module_logger("gnss_geobase_a2rl")) {
    logger->info("initializing GNSS geobase A2RL constraints");
    const std::string config_path = glim::GlobalConfigExt::get_config_path("config_gnss_geobase_a2rl");
    logger->info("gnss_geobase_a2rl_config_path={}", config_path);
    ...
    GlobalMappingCallbacks::on_insert_submap.add(std::bind(&GNSSGeobaseA2RL::on_insert_submap, this, _1));
    GlobalMappingCallbacks::on_smoother_update.add(std::bind(&GNSSGeobaseA2RL::on_smoother_update, this, _1, _2, _3));
  }
  ~GNSSGeobaseA2RL() {
    kill_switch = true;
    thread.join();
  ...
  virtual std::vector<GenericTopicSubscription::Ptr> create_subscriptions() override {
    const auto sub = std::make_shared<TopicSubscription<common_msgs::msg::GNSS>>(gnss_topic, [this](const common_msgs::msg::GNSS::ConstSharedPtr msg) { gnss_callback(msg); });
    return {sub};
  }

  void gnss_callback(const common_msgs::msg::GNSS::ConstSharedPtr& gnss_msg) {
    Eigen::Vector4d gnss_data;

    // Add only if covariance acceptable
    const auto& pos_stddev = gnss_msg->pos_stddev;
    if(pos_stddev.x > 0.5 || pos_stddev.y > 0.5 || pos_stddev.z > 0.5)
      return;

    const double stamp = gnss_msg->timestamp.nanoseconds * 1e-9;
    const auto& pos = gnss_msg->position;
...
extern "C" glim::ExtensionModule* create_extension_module() {
  return new glim::GNSSGeobaseA2RL();
}